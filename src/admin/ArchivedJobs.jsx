...
const ArchivedJobs = () => {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      ...
      try {
        ...
        const response = await api.get("/api/jobs/archived", { params });
  ...
  const handleUnarchive = async (jobId) => {
    setUnarchivingId(jobId);
    try {
      const response = await api.patch(`/api/jobs/${jobId}/unarchive`);
      toast.success(response.data.message || "Job unarchived");
      setJobs((prev) => prev.filter((job) => job._id !== jobId));
      // Optional: If you unarchive the last item on a page > 1, you might want to adjust currentPage
    } catch (err) {
      toast.error(err?.response?.data?.message || "Failed to unarchive job");
    } finally {
    ...
    <div className="p-2">
      ...
      {!loading && !error && jobs.length > 0 && (
        <div className="flex flex-col gap-3">
          {jobs.map((job) => {
            const { day, month } = formatDateBadge(job.archivedAt);
              ...
              >
                ...
                <div className="relative shrink-0">
                  <button
                    type="button"
                    onClick={() =>
                      setConfirmUnarchiveId(
                        confirmUnarchiveId === job._id ? null : job._id,
                      )
                    }
                    disabled={unarchivingId === job._id}
                    title="Unarchive job"
                    className="flex items-center justify-center size-9 rounded-full border border-gray-300 text-gray-500 hover:bg-gray-50 hover:text-black transition disabled:opacity-50"
                  >
                    {unarchivingId === job._id ? (
                ...
                <Link
                  ...
                  className="shrink-0 text-sm font-medium px-4 py-2 rounded-full border border-gray-300 hover:bg-gray-50 transition"
                >
                  View details
                </Link>
              </div>
